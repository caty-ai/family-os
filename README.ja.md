...
<p align="center"><sub>generation: <code>91704ff</code> (2026-09-05T11:55:53Z) · verify: <a href="https://api.github.com/repos/caty-ai/family-os/commits/main">API HEAD</a> · <a href="./status.json">status.json</a></sub></p>
<!-- repo-state:end -->
...
> **家にたとえると** — 掟は、壁に貼られた家訓。縦軸は、それぞれの個室——仕事をやり切る机（Harness）、机まわりの道具（context-kit）、日記と成長のアルバム（Persona）、毎朝届く新聞（X Collector）。横軸は、リビング——家族みんなが読む冷蔵庫の置き手紙（FMA）と、止まった仕事に気づくお手伝いさん（Sitter）。そして Family OS は、この家の**間取り図**です。置き手紙は家族を操作しないし、お手伝いさんは仕事の採点をしません。それはこの家の掟でもあります。
...
| レジストリ・リンク検査ツール | ✅ Linux ／ ✅ macOS（変更のたびに両OSのCIで実走）／ ✅ WSL2（Ubuntu の検査でカバー。clone は ext4 上の `~` 配下へ — `/mnt/c` は不可） | 2026-08-25 |
...
- **家具は替えても、住人は替えません**
  ...
  この地図にあるモジュールはすべて、時代に合わせて取り替えていい「家具」です。私たち自身が作るものも例外ではありません。持ち越すのは、エージェント本人の記憶・経験・関係——それらが形づくる人格のほうです。
