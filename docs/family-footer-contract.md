...
2. Run `python3 -B tools/family_footer.py render-all --checkouts <dir>` (for the full extent — `--org-target` and the SVG PR — see the v3.7 rule below).
...
- `assert_org_svg()` (through its legacy-profile branch, which is what the four committed SVGs use) requires an exact `⏺ <id>` line for **every** module.

A registry PR that adds a `preparing` entry therefore drifts every sibling exactly as a publication does. It already happened once (33 weekly errors after the meetmate registration, cleared in #103) and again on PR #97 (45 drifted targets measured against the live default branches). Rules:
...
2. **Extent.** `render-all --checkouts <dir>` alone is not the sweep. The full extent is:
   ...
   - the four committed SVGs in `caty-ai/.github` — regenerated by that repository's `tools/gen_readme_svg.py` in a **separate `caty-ai/.github` PR**. Full SVG generation from this registry was rejected (see Rejected alternatives) and stays owned by `.github`; this contract only asserts the committed artifacts.
