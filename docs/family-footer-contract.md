...
- `assert_org_svg()` requires an exact `⏺ <id>` line for **every** module in the four committed SVGs.
...
1. **Scope.** Adding, removing, renaming, or reordering an entry in `modules[]` requires the full sibling sweep, independent of `status`. So does changing any rendered field of an existing entry (`status`, `tagline`, `axis`, `name`), because the generated bytes change. `adjacent[]` and `retired_repos[]` do **not**: they are not rendered into the footer table, the org block, or the SVGs, so a non-module needs no `.github` change, no SVG regeneration, and no sibling re-render (#103). The line is "is it a `modules[]` row", not "is it published".
...
4. The sibling-first ordering and the deliberately red window of the publication runbook apply unchanged; this rule only removes the assumption that they are triggered by publication alone.
