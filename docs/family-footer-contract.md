...
2. **Extent.** `render-all --checkouts <dir>` alone is not the sweep. The full extent is:
   - every published sibling's declared README set — `python3 -B tools/family_footer.py render-all --checkouts <dir>` (at the time of writing 11 siblings × 4 languages = 44 files);
