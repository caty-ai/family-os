...
2. Run `python3 -B tools/family_footer.py render-all --checkouts <dir>` (for the full extent — `--org-target`, and the SVG profile rule — see the v3.7 rule below).
...
## v3.7 Sibling Sweep Rule — any `modules[]` change

The publication runbook above is written around a `footer:true` flip. The renderers make no such distinction, so the sweep is owed for **every** change to `modules[]`, whatever the entry's `status`:

- `module_table()` renders every entry of `registry["modules"]`, not only `published_modules()`. A `preparing` entry appears in every sibling footer as a bold, unlinked row with the localized preparing badge.
- `render_org_block()` does the same for the five declared org-profile files.
- `assert_org_svg()` dispatches on the SVG's profile: a **legacy** SVG (one that lists modules) must carry an exact `⏺ <id>` line for **every** module; a **minimal** SVG (one that carries the localized `svg_question` needle) must contain **no** module id at all. The four committed SVGs are minimal today (needle present, zero module ids, verified 2026-09-18 on `caty-ai/.github` HEAD), so they do not drift on a `modules[]` change — they fail only if a module id reappears in them or the question needle is lost (a needle-less SVG falls back to the legacy assertion).

A registry PR that adds a `preparing` entry therefore drifts every sibling exactly as a publication does. It already happened once (33 weekly errors after the meetmate registration, cleared in #103) and again on PR #97 (45 drifted targets measured against the live default branches on 2026-08-21, when the SVGs were still legacy-profile; today the same change drifts 48 sibling files + 5 org-profile files and no SVG). Rules:

1. **Scope.** Adding, removing, renaming, or reordering an entry in `modules[]` requires the full sibling sweep, independent of `status`. So does changing any rendered field of an existing entry (`id`, `name`, `repo`, `status`, `tagline`, `axis`), because the generated bytes — and, for `id`, the legacy-profile SVG module needle — change. `adjacent[]` and `retired_repos[]` do **not**: they are not rendered into the footer table, the org block, or the SVGs, so a non-module needs no `.github` change, no SVG regeneration, and no sibling re-render (#103). The line is "is it a `modules[]` row", not "is it published".
2. **Extent.** `render-all --checkouts <dir>` alone is not the sweep. The full extent is:
   - every published sibling's declared README set — `python3 -B tools/family_footer.py render-all --checkouts <dir>` (at the time of writing 12 published modules × 4 declared languages = 48 files; the map repository is not in `modules[]`);
   - the five org-profile files in `caty-ai/.github` — rendered by the same command **only when `--org-target <dir>` is passed**; without it `render-all` prints `NOTE: the org profile was not rendered; pass --org-target to render it.` and skips them;
   - the four committed SVGs in `caty-ai/.github` — **only while they are legacy-profile**. Under the minimal profile (the committed state) a `modules[]` change needs no SVG work, and adding a module line to a minimal SVG would make `assert_org_svg()` fail with module residue. If the SVGs are ever legacy again, regenerate them with that repository's `tools/gen_readme_svg.py` in a **separate `caty-ai/.github` PR**; full SVG generation from this registry was rejected (see Rejected alternatives) and stays owned by `.github`; this contract only asserts the committed artifacts.
3. **Verify before merging, not after.** PR CI structurally cannot see sibling drift, so the registry PR must not rely on it:
   - the workflow `family links` *does* run on `push` and `pull_request`; what changes is the **step**: "check family footer contract" runs `check --require-reality` only on `schedule` and `workflow_dispatch`, and degrades to `lint` on `push` / `pull_request`. This is deliberate — a remote check on every push would deadlock publication PRs — and it means a green PR run proves the registry shape, never that the siblings match.
   - therefore: render the sweep from the registry PR branch, merge the sibling PRs first (the README PRs, and the `caty-ai/.github` PR carrying the five org-profile files — plus the SVGs only in the legacy case above), then run `python3 -B tools/family_footer.py check --require-reality` locally from the registry PR branch (it reads the siblings' live default branches) and require it green **before** merging the registry PR. A `workflow_dispatch` run of `family links` after the merge records the same result in CI the same day instead of waiting for Monday.
4. The publication runbook's deliberately red window belongs to the `footer:true` flip only. For every other `modules[]` change there is no acceptable red window: rule 3 is the gate, and merging the registry PR first (runbook step 1) is not an option.
