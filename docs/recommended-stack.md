...
| Retrieve | code symbol query | ask the code "who calls this?" and get the current answer, without reading whole files |
...
One retrieve-layer role is about code rather than memory. It sits here because it fails in exactly the same way.
...
We arrived at this role by failing with the alternative. We compared two tools that build a static map of the codebase up front, and both suffered the same pair of defects: **edges that do not exist in the code**, and **maps that go stale the moment the code moves**. A query-time lookup answered a real "who calls this?" question that both prebuilt maps got wrong — because it resolves the question against the code as it is now, not as it was when indexed.

Two honest caveats from daily use:

- **Quality tracks the language server.** Python works essentially unconfigured; Swift answers reference queries only after its build server is wired up, and until then it fails **silently** — an empty result, not an error; shell scripts resolve only down to function granularity
- **The index is only as fresh as the last build or index run.** References into code written minutes ago may not appear until it is rebuilt
